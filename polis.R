...
server = function(input, output, session) {
  ...
  polis_survival_reactive = reactive({
    ...
    if(!is.null(filt_seer_1977)) {
      survival_reactive <- survival_reactive %>% dplyr::filter(Chemotherapy %in% filt_seer_1977)
    }
    filt_seer_2000 <- input$km_seer_2000
    if(!is.null(filt_seer_2000)) {
      survival_reactive <- survival_reactive %>% dplyr::filter(Chemotherapy %in% filt_seer_2000)
    }
    survival_reactive
  })
